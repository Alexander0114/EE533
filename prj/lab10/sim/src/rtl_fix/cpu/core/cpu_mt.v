...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        // FIX v4: sticky halt_seen_once — keep original 2-sighting filter
        // but remove the else clause that clears halt_seen_once.
        // Once B . is seen, halt_seen_once stays set until 2nd sighting.
        if (i_mem_data_i == HALT_ENCODING && !halted[tid_if2]) begin
            if (halt_seen_once[tid_if2]) begin
                halted[tid_if2] <= 1'b1;
                // synthesis translate_off
                $display("[cpu_mt] HALT: tid=%0d t=%0t", tid_if2, $time);
                // synthesis translate_on
            end else begin
                halt_seen_once[tid_if2] <= 1'b1;
            end
        end
        // NO else — halt_seen_once is never cleared by non-B. instructions
    end
end
