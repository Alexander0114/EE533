...
module conv_fifo #(
    ...
    // out_ctrl: replay captured rx_ctrl_r on word 0 (module header),
    // set 8'h80 on last word (EOP: all 8 bytes valid, for output_queues),
    // zero on all other words.
    wire tx_last_word = (tx_state == TX_SEND) && (tx_ptr == tx_end);
    always @(*) begin
        if ((tx_state == TX_SEND) && tx_first_word)
            out_ctrl = rx_ctrl_r;
        else if (tx_last_word)
            out_ctrl = 8'h80;   // EOP: byte 7 is last valid byte (full 8-byte word)
        else
